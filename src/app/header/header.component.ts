...
showicnArrow;
  ...
  onbeforeinstallprompt(e) {
    ...
    this.deferredPrompt = e;
     ...
     //            id:sids,
     //          };
     //          this.apiService.post('update_sessionapp',param1,'').subscribe((result3:any) => {
  ...
  constructor(private notifyService : NotificationService, private deviceService: DeviceDetectorService,private SharedService: GlobalFooService,private apiService:ApiService,private router:Router,private activatedRoute: ActivatedRoute,private toastr: ToastrService) {
    // console.log('>>>>>>>>>>>> ')
this.is_app_exist = localStorage.getItem('green_appinstalled');
    ...
    this.router.events.subscribe((event: Event) => {
        if (event instanceof NavigationStart) {
            ...
            console.log('Route change detected');
             var sids1 = localStorage.getItem('session_gid');
             if(sids1){
              var items = localStorage.getItem('session_items');
               const param1 = {
                snumitems: Number(items)+1,
                id:sids1,
              };
            this.apiService.post('update_sessionoitems',param1,'').subscribe((result3:any) => {
                console.log('session info=>', result3);
              }, error => {
                console.log(error);
              });
    ...
    this.subscription = this.SharedService.getObservable().subscribe((res:any)=>{  
     ...
     console.log('broeeeeeeChrome',this.deviceInfo.browser);
