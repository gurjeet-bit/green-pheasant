 ...
 returnid: any;
  ...
  constructor(private spinner: NgxSpinnerService,private router: Router,public activatedRoute: ActivatedRoute,private notifyService : NotificationService,private apiService:ApiService,private fb: FormBuilder,private toastr: ToastrService) {
    ...
    this.poem_id = activatedRoute.snapshot.paramMap.get('id');  
     ...
     this.router.events.subscribe((event) => {
        // see also 
        console.log('router chnagesss', event)
         if (event instanceof NavigationStart) { 
        console.log('Route change detected1234',event.url);
         const param1 = {
                id: this.returnid,
                user_id: this.user_id,
                last_view_end: new Date(),
              };
              this.apiService.post('updateviewdatainterend',param1,'').subscribe((result3:any) => {
                console.log('user info=>', result3);
              }, error => {
                console.log(error);
              });
      }
  ...
  ngOnInit() {
      ...
      const param = {
                id: this.poem_id,
                user_id: this.user_id,
                last_view_start: new Date(),
              };
              this.apiService.post('updateviewdatainter',param,'').subscribe((result2:any) => {
                console.log('user info=>', result2);
                this.returnid = result2.dataa;
              }, error => {
                console.log(error);
              });
...
interactnupdate(item_id){
    // this.spinner.show();
    const frmData = new FormData(); 
    frmData.append("id", item_id);

    this.apiService.post('update_intraction',frmData,'').subscribe((result:any) => {
